...
def clean_cache():
    # clear Paging fCache because of VideoRecoder I/O CACHE
    print('\n====> CLEAN PAGINGCACHE, DENTRIES, INODES "echo 1 > /writable_proc/sys/vm/drop_caches" \n')
    
    subprocess.run('sync', shell=True)
    subprocess.run('echo 1 > /writable_proc/sys/vm/drop_caches', shell=True) ## For you this Command you should make writable proc file when you run docker.
...
def gen_dataset_using_ffmpeg(input_video_path, output_dir_path):
    ...
    cmd = ['ffmpeg', '-i', input_video_path, '-vf', 'scale=512:512']
    cmd += [output_img_path]
    ...
    for (root, dirs, files) in os.walk(lapa_40_video_base_path):
        ...
        for file in files:
            ...
            if rename_file in OOB_lapa_40:
                output_dir_path = os.path.join(img_base_path, 'LAPA' , 'L_{}'.format(patient_num), full_rename_file)
                createFolder(output_dir_path)

                gen_dataset_using_ffmpeg(os.path.join(root, file), output_dir_path)
                save_log(os.path.join(img_base_path, 'LAPA_database_log.txt'), 'Lapa_40case | Origin_video: {}\t|\t Rename_file: {}\n'.format(os.path.join(root, file), full_rename_file))
                print('Origin_video: {}\t|\t Rename_file: {}\n'.format(os.path.join(root, file), rename_file))
    ...
    for (root, dirs, files) in os.walk(lapa_60_video_base_path):
        ...
        for file in files:
            ...
            if rename_file in OOB_lapa_60:
                ...
                gen_dataset_using_ffmpeg(os.path.join(root, file), output_dir_path)
                save_log(os.path.join(img_base_path, 'LAPA_database_log.txt'), 'Lapa_60case | Origin_video: {}\t|\t Rename_file: {}\n'.format(os.path.join(root, file), full_rename_file))
                print('Lapa_60case | Origin_video: {}\t|\t Rename_file: {}\n'.format(os.path.join(root, file), rename_file))
