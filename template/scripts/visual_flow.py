        ...
        # offline mode
            ...
            for ids, stage in enumerate(STAGE_LIST):
                ...
                if args.stage_flag: # STAGE 2 단계 학습이라면,
                    ...
                    stage_hem_base_path = os.path.join(mc_assets_save_path['robot'], 'hem_assets', args.model, 'WS={}-IB={}-seed={}'.format(int(args.WS_ratio), int(args.IB_ratio), args.random_seed), 'hem_extract_mode={}-ver={}-top_ratio={}-n_dropout={}.csv'.format(args.hem_extract_mode + '_lower', 1, int(args.top_ratio * 100) , args.n_dropout))
