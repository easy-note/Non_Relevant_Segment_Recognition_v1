...
def inference_main(args):
    ...
    inference_assets = prepare_inference_aseets_vihub_gangbuk(case=case , anno_ver=anno_ver, inference_fold=inference_fold, save_path=inference_assets_save_path)
    ...
    for idx in range(patients_count): # per patients
        ...
        for video_path_info in patient['path_info']: # per videos
            ...
            # 22.01.05 [hg buxfix] - matching to target_frame_idx_list length with gt_list
            fitted_length = len(predict_list)

            # save predict list to csv
            predict_csv_path = os.path.join(each_videos_save_dir, '{}.csv'.format(video_name))
            predict_df = pd.DataFrame({
                            'frame_idx': target_frame_idx_list[:fitted_length],
                            'predict': predict_list,
                            'gt': gt_list, # it will be fitted in evaluator.get_assets()
                            'target_img': target_img_list[:fitted_length],
                        })
            predict_df.to_csv(predict_csv_path)
...
if __name__ == '__main__':
    ...
    if __package__ is None:
        ...
        from core.utils.misc import save_dict_to_csv, prepare_inference_aseets_vihub_gangbuk, get_inference_model_path, \
