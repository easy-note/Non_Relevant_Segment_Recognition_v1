...
def inference_main(args):
    ...
    details_results_path = os.path.join(args.restore_path, 'inference_results_lapa')
    ...
    report_path = os.path.join(args.restore_path, 'Report_lapa.json')
...
def extract_hem_assets(extract_args, offline_methods, save_path): # save_path 는 저장하는 ~/TB_log/version_0 에서의 ~
    ...
    for train_stage in extract_stage:
        ...
        dataset_type = extract_args.dataset
...
def inference_lapa(args):
    args.train_stage = 'hem_train'
    args.hem_extract_mode = 'hem-softmax_diff_small-offline'

    # args.restore_path 설정해줘어야햄!
    args, experiment_summary, patients_CR, patients_OR = inference_main(args)
