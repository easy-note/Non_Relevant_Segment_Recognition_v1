import core.config.patients_info_vihub_gangbuk as patients_info_vihub_gangbuk
...
class InferenceAssets_vihub_gangbuk():
    """
    ...
    def _get_patient_video(self, case, fold):
        ...
        if case == 'ROBOT':
            ...
            patient_video_dict = self._parsing_patient_video(patients_info_vihub_gangbuk.val_videos[fold], patients_info_vihub_gangbuk.video_details['lapa'])

        return patient_video_dict        
    
    def _get_assets_sheet(self, case, anno_ver):
        assets_sheet_dir = 'dummy'
        oob_assets_etc = OOBAssets_vihub_gangbuk(assets_sheet_dir, case, anno_ver)
...
class OOBAssets_vihub_gangbuk():
    ...
    def _get_video_details(self):
        if self.case == 'ROBOT':
            ...
            return patients_info_vihub_gangbuk.video_details['lapa']

    def _get_annotation_root_path(self):
        if self.anno_ver == '3':
            return assets_info.annotation_path['annotation_vihub_gangbuk_base_path']

    def _get_img_db_root_path(self):
        if self.case == 'ROBOT':
            pass
        elif self.case == 'LAPA':
            return assets_info.img_db_path['vihub_gangbuk']

    def _get_video_root_path(self):
        if self.case == 'ROBOT':
            pass
        if self.case == 'LAPA':
            return assets_info.video_path['vihub_gangbuk'][0]

    def _get_video_sheet_for_lapa(self, video_root_path, use_video_list):

        print('\n{}'.format('=====' * 10))
        print('\t ===== GET VIDEO PATH FOR LAPA =====')
        print('{}\n'.format('=====' * 10))
    ...
    def _get_img_db_sheet(self, img_db_root_path, use_video_list):
        ...
        for video_name in use_videos :
            ...
            # re-define used videe name for parsing img db
            op_method, patient_idx, video_channel, video_slice_no = video_name.split('_') # L_1_empty_01
            redefine_video_name = '_'.join([op_method, patient_idx, video_slice_no]) # L_1_01

            idx = self._return_idx_is_str_in_list(redefine_video_name, all_DB_path)
    ...
    def get_assets_sheet(self): # only get assets sheet, if you want to save, using make_assets_sheet()
        ...
        if self.case == 'ROBOT':
            ...
            vihub_gangbuk_case_root_path = self._get_video_root_path()
            ...
            video_sheet = self._get_video_sheet_for_lapa(vihub_gangbuk_case_root_path, video_details)
